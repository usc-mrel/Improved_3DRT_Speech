...
function [Recon, coilmap, reconInfo] = recon3dsos_3d(matfname, param)
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Initialization
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if folder_index == 1       % original method
        ...
        first_bottom_z_idx = first_bottom_z_idx(1);
    else 
    ...
    for n_frame = 1 : max_nframes    
        ...
        if folder_index == 1 
            t_idx = t_idx + first_bottom_z_idx - 1; 
        end
    ...
    for ij = 1 : cat_num
        ...
        if ij > 1 && ij < cat_num 
           ttt = (ij-1) * nframes : ij * nframes + 1;
           ddd = (ij-1) * nframes +1 : ij * nframes;
        end
        if ij == cat_num
            if mod(max_nframes,nframes) ~= 0
               ttt = (max_nframes - nframes_end) : max_nframes;
               ddd = (max_nframes - nframes_end) + 1 : max_nframes;
            else               
               ttt = (ij-1) * nframes : ij * nframes;
               ddd = (ij-1) * nframes +1 : ij * nframes;
            end
        end
